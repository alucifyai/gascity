...
func (s *BdStore) ListByLabel(label string, limit int) ([]Bead, error) {
	args := []string{"list", "--json", "--label=" + label, "--all", "--limit", fmt.Sprintf("%d", limit), "--include-infra"}
	out, err := s.runner(s.dir, "bd", args...)
	if err != nil {
		return nil, fmt.Errorf("bd list: %w", err)
	}
...
func (s *BdStore) ListByAssignee(assignee, status string, limit int) ([]Bead, error) {
	args := []string{"list", "--json", "--assignee=" + assignee, "--status=" + status, "--limit", fmt.Sprintf("%d", limit), "--include-infra"}
	out, err := s.runner(s.dir, "bd", args...)
	if err != nil {
		return nil, fmt.Errorf("bd list: %w", err)
	}
